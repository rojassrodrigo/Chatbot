...
const handleSubmit = async (event: React.FormEvent) => {
  ...
  return createPortal((
    ...
    <div className='fixed bottom-4 rigth-4'>
      {
        isCollapsed ? (
          <button 
              className='bg-blue-500 text-white pg-2 rounded tl-tg' 
              onClick={() => toggleCollapsed(false)}>
              Abri Chat
          </button>
        ) : (
            <div>
                      <button className='text-xl rounded-full bg-blue-800 w-8 h-8 absolute -right-4 -top-5' onClick={() => toggleCollapsed(true)}>x</button>
                        ...
                        <div className='flex flex-col gap-4 m-auto max-w-lg border border-gray-400 p-4 rounded-md'>
                          ...
                          <form className='flex items-center' onSubmit={handleSubmit}>
                            ...
                            />
                            <button className={`px-4 py-2 bg-blue-500 rounded-lg rounded-l-none ${isLoading ? "bg-blue-300" : "bg-blue-500" }`} type='submit'>Enviar</button>
                          </form>
                        </div>
            </div>
        )
